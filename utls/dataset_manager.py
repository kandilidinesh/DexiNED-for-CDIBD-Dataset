...
#Not using Augmentation for CDIBD dataset for now.
...
def data_parser(args):
    ...
    if args.model_state=='train':
        ...
        train_files_name = args.train_list #Found inside base_dataset_dir
...
def get_batch(arg,file_list, batch=None, use_batch=True):
    ...
    if use_batch:
        ...
        for idx, b in enumerate(batch):
            ...
            # x = x[:, :, arg.channel_swap] # While using OpenCV for converting channel swapping is not necessary
            ...
            if arg.train_dataset.lower()=='cdibd':
                y = image_normalization(y)
                y[y < 51] = 0  # first 100
                # y[y >= 100] = 255.0 #  first 100 and this uncomented [v4]
            else:
